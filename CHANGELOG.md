...
### Removed
- **Monthly sold hours** — `MonthSelling` model record, `WorkPeriodTracker.setMonthSelling()` / `getMonthSelling()` / `getMonthSellings()` / `setMonthSellings()`, and `HourSellingView` monthly mode (toggle buttons, `buildMonthlyCards()`, `buildMonthCard()`). The monthly mode showed cards but had no effect on the Dashboard or Extras charts; keeping only yearly sold hours avoids confusion. `MonthSelling` entry also removed from `reflect-config.json` and `selling.mode.monthly` key removed from all 7 i18n files.
